...
class GroupChat extends Component {
  ...
  sendMessage = async (inputMsg = "", attachments = []) => {
    ...
    const data = {
      ...
      message:
        inputMsg === ""
          ? `${name}: [${attachments[0].type || "file"}]`
          : `${name}: ${inputMsg}`, // Message content
      attachments, // annex
    ...
    const response = await request.socketEmitAndGetResponse(
      ...
      data,
      error => {
        notification("Information failed to be sent", "error", 2);
      }
    );
    addGroupMessages({
      allGroupChats,
  ...
  joinGroup = async () => {
    ...
    const response = await request.socketEmitAndGetResponse(
      ...
      error => {
        notification("Addition failure", "error", 1.5);
    ...
    if (messages.length > 1) {
      ...
      lastContent = {
        ...groupInfo,
        message: "Join the group successfully and start chatting.:)",
  ...
  _showLeaveModal = () => {
    this.setState(state => ({
      showLeaveGroupModal: !state.showLeaveGroupModal
    }));
  };
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      relatedCurrentChat ||
      match.params.to_group_id !== this.chatId ||
      this._sendByMe
    ) {
      this._sendByMe = false;
      return true;
  ...
  _clickPersonAvatar = user_id => {
    ...
    const personalInfo = members.filter(
  ...
  componentDidMount() {
    ...
    // (Product Design) When searching for groups that have not been added, click to go to the group content, request the group content, and avoid adding the wrong group.
    if (!chatItem) {
      window.socket.emit(
        "getOneGroupItem",
        { groupId: this.chatId, start: 1 },
        groupMsgAndInfo => {
  ...
  render() {
    ...
    return (
      <div className="chat-wrapper">
        ...
        {showGroupChatInfo && (
          <div
            onClick={() => this._showGroupChatInfo(false)}
