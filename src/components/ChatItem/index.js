  ...
  sharePersonalCard = shareObj => {
    ...
    return (
      ...
      >
        ...
        <p className="shareButton">Click to add as a contact</p>
      </div>
    );
  };
  ...
  textRender = msg => {
    ...
    if (isShareUrl) {
      ...
      if (shareObj.to_group_id) {
        return (
          <div className="msg-render">{this.shareGroupCard(shareObj)}</div>
        );
      }
      if (shareObj.user_id) {
        return (
          <div className="msg-render">{this.sharePersonalCard(shareObj)}</div>
        );
      }
    }
            ...
            (Rule, ruleNumber) => (
              <Emoji
                ...
                fallback={(emoji, props) =>
                  emoji ? `:${emoji.short_names[0]}:` : props.emoji
                }
              />
            )
          )}
