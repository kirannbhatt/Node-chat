...
export default class InputArea extends Component {
  constructor(props) {
    ...
    this._onPaste = props.isRobotChat
      ? () => {}
      : debounce(this._paste, 2000, true);
  }

  componentWillMount() {
    if (/group_chat/.test(window.location.href)) {
      this._placeholder =
        "Support Enter to send information/Paste the map/@Others";
    } else if (this.props.isRobotChat) {
      this._placeholder = "Support Enter to send messages";
    } else {
      // private chat
      this._placeholder = "Support Enter to send information/Paste the map";
    }
  }
  ...
  _selectSomeOneOrNot = () => {
    ...
    if (groupMembers && groupMembers.length > 1) {
      ...
      const relatedMembers = filterText
  ...
  componentDidMount() {
    if (this.props.shareData) {
      this._sendMessage({
        message: `::share::${JSON.stringify(this.props.shareData)}`
      });
  ...
  _onSelectFile = e => {
    ...
    reader.onloadend = event => {
      ...
      if (file.size > limitSize) {
        notification("The file can't exceed 2MB.!", "warn", 2);
        return;
      }
  ...
  render() {
    ...
    return (
      <div className="input-msg">
        {showEmojiPicker && (
          <div onClick={this._clickShowEmojiPicker} className="mask" />
        )}
        {showEmojiPicker && (
          <Picker
        ...
        <div className="left" style={this.props.isRobotChat ? robotStyle : {}}>
          <svg
            onClick={this._clickShowEmojiPicker}
