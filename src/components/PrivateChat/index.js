...
export default class PrivateChat extends Component {
  ...
  sendMessage = async (inputMsg = "", attachments = []) => {
    ...
    const data = {
      ...
      to_user: this.friendId, // Other id
      avatar, //Own avatar
      name,
      github_id,
      message:
  ...
  addAsTheContact = async () => {
    ...
    if (this.chatId === this._userInfo.user_id) {
      notification("Can't add yourself as a contact", "error", 2);
      return;
    }
    ...
    const dataInHomePageList = {
      ...
      message: "Add a contact successfully, send me a message:)",
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      relatedCurrentChat ||
      match.params.user_id !== this.chatId ||
      this._sendByMe
    ) {
      this._sendByMe = false;
      return true;
