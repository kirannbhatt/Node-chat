    ...
    return (
      <div className="userInfo">
        ...
        {website && (
          <p
            className="website"
            onClick={() => _openUrl(website)}
          >{`website: ${website}`}</p>
        )}
        {github && (
          <p
            className="github"
            onClick={() => _openUrl(github)}
          >{`github: ${github}`}</p>
        )}
        {showContactButton && (
          <Button
            ...
            clickFn={goToChat}
            value="Private chat with this contact"
          />
        )}
        {isContact && (
          <Button
...
class PersonalInfo extends Component {
    ...
    window.socket.emit(
      ...
      res => {
        if (res.code === 200) {
          ...
          this.props.hide();
          notification("Delete contact successfully", "success", 2);
        }
      }
    );
  };
  ...
  get isContact() {
    return (
      ...
      this.props.homePageList.find(
        e => e.user_id === this.props.userInfo.user_id
      )
    );
  }
